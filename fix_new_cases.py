#!/usr/bin/env python3
"""Fix new diagnostic cases to use low_idle.lambda (sensor) instead of calculated_lambda."""
...
# Remove the problematic new cases we just added (by our IDs)
ids_to_remove = [
    'late_ignition_timing', 'ignition_failure_no_spark', 'fuel_delivery_lean',
    'fuel_delivery_rich_injector_leak', 'egr_stuck_open_idle', 'catalyst_failure_high_nox',
    'cold_start_over_enrichment', 'mass_airflow_underreport', 'o2_sensor_lazy',
    'ignition_misfire_high_hc_o2', 'system_rich_negative_trims', 'exhaust_leak_false_lean',
    'vacuum_leak_differential', 'cat_inefficiency_perfect_lambda', 'engine_wear_oil_burning'
]
kb['diagnostic_matrix'] = [c for c in kb['diagnostic_matrix'] if c['case_id'] not in ids_to_remove]

# Revised cases using only low_idle.lambda and gas signs
new_cases = [
    {
...
print(f"Added {added} revised cases.")
